...
class InteractiveFittingGUI(QWidget):
    ...
    def setup_ui(self):
        """Setup the user interface with left/right split layout like batch_tkinter"""
        ...
        left_panel.setStyleSheet("background-color: #F8F9FA; border-right: 2px solid #DEE2E6;")
        left_layout = QVBoxLayout(left_panel)
        left_layout.setContentsMargins(5, 5, 5, 5)
        left_layout.setSpacing(5)

        # Right panel - Plot and Info
        right_panel = QWidget()
        ...
        self.setup_background_panel(left_layout)
        self.setup_smoothing_panel(left_layout)
        self.setup_results_panel(left_layout)
        left_layout.addStretch()

        # Setup right panel sections
    ...
    def setup_control_panel(self, parent_layout):
        """Setup control panel - grid layout for left sidebar"""
        control_widget = QWidget()
        control_widget.setAutoFillBackground(True)
        control_widget.setStyleSheet("QWidget { background-color: #FFFFFF; border: 1px solid #DEE2E6; border-radius: 4px; }")
        ...
        row1.addWidget(load_btn, 3)

        prev_btn = QPushButton("◀")
        prev_btn.setFixedWidth(45)
        prev_btn.setStyleSheet(btn_style + f"background-color: {self.palette['info']}; color: white;")
        prev_btn.clicked.connect(self.prev_file)
        row1.addWidget(prev_btn, 1)

        next_btn = QPushButton("▶")
        next_btn.setFixedWidth(45)
        next_btn.setStyleSheet(btn_style + f"background-color: {self.palette['info']}; color: white;")
        next_btn.clicked.connect(self.next_file)
        row1.addWidget(next_btn, 1)
        
        control_layout.addLayout(row1)

        # Row 2: Fit and Auto Detect
        row2 = QHBoxLayout()
        row2.setSpacing(6)
        ...
        # Row 5: Overlap and Batch
        row5 = QHBoxLayout()
        row5.setSpacing(6)
        
        overlap_btn = QPushButton("🔀 Resolve Overlap")
        overlap_btn.setStyleSheet(btn_style + f"background-color: {self.palette['info']}; color: white;")
        overlap_btn.clicked.connect(self.resolve_overlaps)
        row5.addWidget(overlap_btn)
    ...
    def setup_background_panel(self, parent_layout):
        ...
        bg_widget.setStyleSheet("QWidget { background-color: #E7F5FF; border: 1px solid #B8DAFF; border-radius: 4px; }")  # Light blue tint
    ...
    def setup_smoothing_panel(self, parent_layout):
        ...
        smooth_widget.setStyleSheet("QWidget { background-color: #F3F0FF; border: 1px solid #D4C5F9; border-radius: 4px; }")  # Light purple tint
    ...
    def setup_results_panel(self, parent_layout):
        ...
        results_widget.setStyleSheet("QWidget { background-color: #FFF9DB; border: 1px solid #FFE69C; border-radius: 4px; }")  # Light yellow tint
    ...
    def setup_info_panel(self, parent_layout):
        ...
        info_widget = QWidget()
        info_widget.setFixedHeight(90)
        info_widget.setAutoFillBackground(True)
        info_widget.setStyleSheet("QWidget { background-color: #E7F5FF; border: 1px solid #B8DAFF; border-radius: 4px; }")  # Light blue tint
