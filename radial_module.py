...
from sector_config_dialog import SectorConfigDialog
...
class AzimuthalIntegrationModule(GUIBase):
    ...
    def setup_integration_module(self, parent_layout):
        ...
        bin_btn_layout = QHBoxLayout(bin_btn_row)
        bin_btn_layout.setContentsMargins(10, 5, 10, 5)
        ...
        """)
        self.bin_config_btn.clicked.connect(self.open_bin_config)
        bin_btn_layout.addWidget(self.bin_config_btn)
        
        self.bin_status_label = QLabel("No bins configured")
        self.bin_status_label.setFont(QFont('Arial', 9))
        self.bin_status_label.setStyleSheet("color: #999999;border: none;")
        ...
        settings_layout.addWidget(bin_btn_row)
        
        # Multiple Sector Configuration Button
        sector_btn_row = QWidget()
        sector_btn_row.setStyleSheet(f"background-color: {self.colors['card_bg']};")
        sector_btn_layout = QHBoxLayout(sector_btn_row)
        sector_btn_layout.setContentsMargins(10, 5, 10, 5)
        ...
        """)
        self.sector_config_btn.clicked.connect(self.open_sector_config)
        sector_btn_layout.addWidget(self.sector_config_btn)
        
        self.sector_status_label = QLabel("No sectors configured")
        self.sector_status_label.setFont(QFont('Arial', 9))
        self.sector_status_label.setStyleSheet("color: #999999;border: none;")
        ...
        settings_layout.addWidget(sector_btn_row)

        # Output formats
        formats_label = QLabel("Output Formats:")
        formats_label.setFont(QFont('Arial', 9, QFont.Weight.Bold))
        formats_label.setStyleSheet(f"color: #666666; background-color: {self.colors['card_bg']};border:none;")
        settings_layout.addWidget(formats_label)

        # Format checkboxes in a grid (2 columns)
        formats_grid = QWidget()
        formats_grid.setStyleSheet(f"background-color: {self.colors['card_bg']};")
        formats_grid_layout = QVBoxLayout(formats_grid)
        formats_grid_layout.setContentsMargins(10, 5, 10, 5)
        ...
        for i in range(0, len(format_options), 3):
            ...
            label1, var1, default1 = format_options[i]
            ...
            if i + 1 < len(format_options):
                label2, var2, default2 = format_options[i + 1]
        ...
        settings_layout.addWidget(formats_grid)
        
        # Visualization options
        viz_label = QLabel("Visualization Options:")
        viz_label.setFont(QFont('Arial', 9, QFont.Weight.Bold))
        viz_label.setStyleSheet(f"color: #666666; background-color: {self.colors['card_bg']};border:none;")
    ...
    def open_bin_config(self):
        """Open bin mode configuration dialog"""
        # Use root window as parent (stored during __init__)
        parent_widget = self.root if hasattr(self, 'root') else None
        dialog = BinConfigDialog(parent_widget)
        
        # Set existing bins if any
        if self.bins:
            dialog.bins = self.bins.copy()
            dialog.update_table()
        
        def on_bins_configured(bins):
            self.bins = bins.copy()
            self.bin_mode_enabled = len(bins) > 0
            self.log(f"Bin mode configured: {len(bins)} bins")
            self.bin_status_label.setText(f"✓ {len(bins)} bins configured")
            self.bin_status_label.setStyleSheet("color: #4CAF50; font-weight: bold;")
        
        dialog.bins_configured.connect(on_bins_configured)
        dialog.exec()
    
    def open_sector_config(self):
        """Open multiple sector configuration dialog"""
        # Use root window as parent (stored during __init__)
        parent_widget = self.root if hasattr(self, 'root') else None
        dialog = SectorConfigDialog(parent_widget)
        
        # Set existing sectors if any
        if self.sectors:
            dialog.sectors = self.sectors.copy()
            dialog.update_table()
        
        def on_sectors_configured(sectors):
            self.sectors = sectors.copy()
            self.log(f"Multiple sectors configured: {len(sectors)} sectors")
            self.sector_status_label.setText(f"✓ {len(sectors)} sectors configured")
            self.sector_status_label.setStyleSheet("color: #4CAF50; font-weight: bold;")
        
        dialog.sectors_configured.connect(on_sectors_configured)
        dialog.exec()
    ...
    def _do_integration(self):
        ...
        try:
            ...
            if use_bin_mode and use_multiple_sectors:
                self.log("⚠ Warning: Cannot use both Bin Mode and Multiple Sectors simultaneously")
                self.log("Using Bin Mode only")
                use_multiple_sectors = False
            
            if use_bin_mode:
                self.log(f"✓ Bin Mode enabled: {len(self.bins)} bins configured")
            elif use_multiple_sectors:
                self.log(f"✓ Multiple Sector Mode enabled: {len(self.sectors)} sectors configured")
            else:
            ...
            for i, h5_file in enumerate(input_files, 1):
                ...
                try:
                    if use_bin_mode:
                        # Bin mode: Process each bin separately
                        self.log(f"Processing ({i}/{len(input_files)}): {basename} - {len(self.bins)} bins")
                        for j, bin_data in enumerate(self.bins, 1):
                            ...
                            self.log(f"  [{j}/{len(self.bins)}] Bin: {bin_name} ({azim_start}° - {azim_end}°)")
                        ...
                        self.log(f"  ✓ Completed all bins for {basename}")
